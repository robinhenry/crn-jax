...
class Params:
    ...
    @classmethod
    def easy(cls) -> "Params":
        return cls(beta_0=0.0, beta_1=5.0, K=1.0, n=1.0, delta=1.0)

    @classmethod
    def hard(cls) -> "Params":
        return cls(beta_0=0.0, beta_1=5.0, K=1.0, n=2.0, delta=1.0)


def propensities_fn(params: Params) -> Callable[[State, Array], Array]:
    def f(state: State, _u: Array) -> Array:
        X = state.x[0]
        repression = 1.0 - hill_function(X, params.K, params.n)
        return jnp.array(
            [
