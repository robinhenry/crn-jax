...
| [`03_grn_models.py`](03_grn_models.py) | Using the `crn_jax.models` library: pull two pre-built systems (the repressilator oscillator and a bistable toggle switch) and simulate each with a single `sample_trajectories(...)` call. Demonstrates the canonical-model convenience API. |
