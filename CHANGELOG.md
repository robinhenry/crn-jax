...
- `sample_trajectories` is the package-level entry point:
  `crn_jax.models.sample_trajectories(model, key, x0, ...)`. The caller always
...
- Each model module is now just the math (`SPECIES`, `Params`,
  ...
  and `_build_simulator` wrappers from earlier drafts are removed.
- `sample_trajectories` does not accept a `u_dist`: every model in the new
